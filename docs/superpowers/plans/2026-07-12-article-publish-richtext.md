...
> **For agentic workers:** REQUIRED SUB-SKILL: 使用 superpowers:subagent-driven-development（推荐）或 superpowers:executing-plans 逐任务实施本计划。步骤用 checkbox（`- [ ]`）语法追踪。
...
- [ ] **Step 3: 创建 RichEditor 主组件（含 EditorContext + ref）**
...
- [ ] **Step 5: 导出 RichEditor（如有 barrel）**
...
- [ ] **Step 1: 改写 publish/index.tsx —— 状态、登录拦截、载入回填**
...
- [ ] **Step 1: 实现 useDraftAutosave hook**
...
- [ ] **Step 2: 发布页接入 useDraftAutosave + SaveStatus 状态字**
...
- [ ] 富文本编辑：加粗/H2 标题/项目列表常驻工具栏生效
- [ ] 「更多」面板：斜体/下划线/删除线/有序列表/对齐/引用/缩进生效
- [ ] 预设色板：文字色/背景色应用；S/M/L 字号切换
- [ ] 工具栏点击不丢光标（catchtouchstart 生效）
- [ ] 图片插入：chooseImage → 上传 → 光标处插图；上传中禁重复
- [ ] 话题：`#话题` 收集进 topics 提交
- [ ] 封面：含图取首图为 cover；无图 cover 空且可提交
- [ ] 发布：标题/正文非空校验；发布成功进详情
- [ ] 自动保存：输入 1800ms 后「保存中…→草稿已保存」；纯空白不建草稿
- [ ] 竞态：快速输入后立即发布，不产生重复草稿/丢失
- [ ] 草稿箱：分页展示、空态、上拉加载
- [ ] 草稿箱继续编辑：回填标题/正文/话题
- [ ] 删除草稿：二次确认后移除
- [ ] 编辑已发布帖子：保存后仍为 PUBLISHED
- [ ] 未登录进发布页触发登录，未登录不可提交

- [ ] **Step 5: 记录冒烟结果，提交（如有微调）**
