...
> **给执行者：** 必需子技能 —— 使用 superpowers:subagent-driven-development（推荐）或 superpowers:executing-plans 按任务逐个实现本计划。步骤用 checkbox（`- [x]`）语法追踪。
...
- [x] **Step 1: 扩展 PostCard 支持 action**
...
- [x] **Step 1: 引入 isUnauthorized，改写 submit catch**
...
- [x] **Step 2: SaveStatus 文案接入 expired 态**
...
- [x] **Step 2: 引入 isUnauthorized 并改 catch**
...
- [x] **Step 4: 微信开发者工具/真机冒烟清单**

逐项人工核对（在微信开发者工具或真机）：

- [x] 详情页加载中展示结构化骨架（标题条+头像行+正文块），加载完成替换真实内容
- [x] 草稿卡片右上角删除按钮：点击弹二次确认，确认后从列表移除
- [x] 点击草稿卡片主体（非删除按钮）进入编辑页，不误触删除、不误跳详情
- [x] 回归：信息流/我的帖子/收藏卡片无 action，展示与改动前一致
- [x] 发布时 401：弹「登录已过期」模态，确认后触发重登，title/正文/话题保留
- [x] 自动保存时 401：不弹窗，保存状态字显示「登录已过期，内容已保留」，编辑不被打断
- [x] 个人中心资料卡出现「设置」按钮，点击右滑入抽屉+遮罩
- [x] 点击遮罩或「关闭」收起抽屉
- [x] 抽屉三项：主题（禁用态「即将上线」，点击 toast 提示不切换）、草稿箱（跳转草稿页）、退出登录（置底，二次确认后登出回未登录态）
- [x] 个人中心页面底部不再有退出登录、Tab 上方不再有草稿箱入口（已迁进抽屉）
- [x] 抽屉打开时背景不可滚动穿透

- [x] **Step 5: 提交验证记录（如流程需要）**
