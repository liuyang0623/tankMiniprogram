...
- [ ] **Step 1: 用 Taro CLI 脚手架初始化到临时目录再合并**
...
- [ ] **Step 5: 首页占位 + TabBar 路由（app.config.ts）**
...
- [ ] **Step 2: 配置 Tailwind 主题（注入设计 token）**
...
- [ ] **Step 2: Transition（fadeInUp 包装）+ Toast（读 uiStore，Task 7 后接线）**
...
export interface PostImage { id: number; url: string; sortOrder: number }
export interface Topic { id: number; name: string }
export interface Post {
  id: number; content: string; status: PostStatus
  viewCount: number; likeCount: number; commentCount: number
  author: PostAuthor; images: PostImage[]; topics: Topic[]
  createdAt: string; updatedAt: string
}
export interface Comment { id: number; postId: number; userId: number; content: string; parentId?: number; replies?: Comment[]; createdAt: string }
export interface Paginated<T> { list: T[]; total: number; page: number; pageSize: number }
export interface AuthResponse { token: string; user: User }
```
> 注：字段名以 go-service handler 实际 JSON 返回为准（GORM 默认 CamelCase/自定义 json tag），实现时对齐后端 Swagger（`/api/docs`）。
...
describe('request', () => {
  ...
  it('成功时解包 data', async () => {
    ;(Taro.request as any).mockResolvedValue({ statusCode: 200, data: { code: 0, data: { id: 1 }, message: 'ok' } })
    await expect(request({ url: '/posts/1' })).resolves.toEqual({ id: 1 })
  })
  it('业务 code 非 0 抛 ApiError', async () => {
...
- [ ] **Step 3: 实现 errors.ts 与 request.ts**
```ts
// errors.ts
...
export async function request<T = any>(opts: Opts): Promise<T> {
  ...
  if (res.statusCode >= 200 && res.statusCode < 300 && body && (body.code === 0 || body.code === undefined)) {
...
- [ ] **Step 2: 运行确认失败** — Run: `bunx vitest run src/store/__tests__/auth.test.ts` Expected: FAIL

- [ ] **Step 3: 实现 auth.ts / ui.ts**
```ts
// auth.ts
...
- [ ] **Step 5: 接线 Toast 组件订阅 uiStore，Commit**
```bash
...
- [ ] **Step 1: authRequest 封装（自动带 token + 401 清态）**
```ts
import { request } from './request'
